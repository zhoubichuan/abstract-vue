    ...
    扩展属性
      ...
      <table-page
        ...
        <template slot-scope="scope">
            <el-link type="primary" @click="showViewModal(scope.$index, scope.row)">{{scope.row.productName}}</el-link>
        </template>
        </el-table-column>
        <el-table-column
          sortable
...
export default {
  ...
  computed: {
    pageSate () {
        return this.$DataInstance.tabsData.data.state
      },
      disableds () {
        let result = {
          code: false,
        ...
        if (this.pageSate !== 'show') {
          ...
          let oldFormData = this.$DataInstance.tabsData.data.data
