...
import FormMixin from '@/mixin/FormMixin.js'
  export default {
    ...
    methods: {
      handleInit () {
        this.baseInfoForm = this.$set(
          this, 'baseInfoForm',
          window._.cloneDeep(this.$DataInstance.tabsData.data)
          )
      },
      ...
      handleAdd () {
        ...
        this.$api.createDataInstance(params).then(res => {
          ...
          this.$findComponentUpward(this, 'DataInstance').$emit('handleSearch')
          } else {
      ...
      handleEdit () {
        ...
        this.$api.updateDataInstance(params).then(res => {
          if (res.status === 200) {
            ...
            let parent = this.$findComponentUpward(this, 'DataInstance')
    ...
    computed: {
      pageSate () {
        return this.$DataInstance.tabsData.state
      },
      disableds () {
        let result = {
          code: false,
        ...
        if (this.pageSate !== 'show') {
          ...
          let oldFormData = this.$DataInstance.tabsData.data
